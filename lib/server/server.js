...
Meteor.methods({
  'raix:push-update': function(options, userId) {
    if (Push.debug) {
  ...
  'raix:push-setuser': function(id, userId) {
    check(id, String);

    if (Push.debug) {
      console.log('Push: Settings userId "' + userId + '" for app:', id);
    }
    // We update the appCollection id setting the Meteor.userId
    var found = Push.appCollection.update({ _id: id }, { $set: { userId: userId } });
